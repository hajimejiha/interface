	...
	for i in prange(grid_x):
		for j in range(grid_y):
			for k in range(grid_z):
				if math.sqrt( ((i*dx+x0)-x)**2.0 + ((j*dx+y0)-y)**2.0 + ((k*dx+z0)-z)**2.0 ) < K * dx:
					temp[i,j,k] = (2.0*K*dx)**-3.0 \
					*(1.0 + math.cos(math.pi / (K * dx) * (i*dx+x0)-x)) \
					*(1.0 + math.cos(math.pi / (K * dx) * (j*dx+y0)-y)) \
					*(1.0 + math.cos(math.pi / (K * dx) * (k*dx+z0)-z))
	return temp
